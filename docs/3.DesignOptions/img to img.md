...
만약 트레이닝 사이즈보다 큰 이미지를 생성하려고 시도한다면, <br>예상치 못한 결과가 발생할 수 있습니다.
> 예를 들어, 모델이 하나의 건물 대신 두 개의 건물을 만들거나, <br>사람의 형태가 이상하게 나타나는 경우 등이 있습니다.
<br><br>이러한 현상은 모델이 더 큰 이미지에 대해 최적화되지 않았기 때문에 발생합니다.
